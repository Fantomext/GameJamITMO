...
void AdditionalLights_float(float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView, out float3 Diffuse, out float3 Specular, out float ShadowAttenuation)
{
    ...
    for (int i = 0; i < pixelLightCount; ++i)
    {
        ...
        float shadowAtten = light.distanceAttenuation * light.shadowAttenuation;
        
        diffuseColor += LightingLambert(attenuatedLightColor * shadowAtten, light.direction, WorldNormal);
        specularColor += LightingSpecular(attenuatedLightColor, light.direction, WorldNormal, WorldView, White, Smoothness);
        shadowAttenuation += shadowAtten;
    }
#endif
